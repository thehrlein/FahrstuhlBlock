...
class FixedGridLayoutManager : RecyclerView.LayoutManager() {
    ...
    override fun onLayoutChildren(recycler: RecyclerView.Recycler, state: RecyclerView.State) {
             ...
             */if (!state.isPreLayout && verticalSpace > totalRowCount * mDecoratedChildHeight) {
                ...
                if (mFirstVisiblePosition + mVisibleColumnCount > state.itemCount) {
                    mFirstVisiblePosition =
                        Math.max(state.itemCount - mVisibleColumnCount, 0)
                    childLeft = paddingLeft
                }
            }
        ...
        if (!state.isPreLayout && !recycler.scrapList.isEmpty()) {
            val scrapList: List<RecyclerView.ViewHolder> =
                recycler.scrapList
            val disappearingViews: HashSet<View> = HashSet<View>(scrapList.size)
    ...
    override fun onAdapterChanged(
        oldAdapter: RecyclerView.Adapter<RecyclerView.ViewHolder>?,
        newAdapter: RecyclerView.Adapter<RecyclerView.ViewHolder>?
    ) {
    ...
    private fun fillGrid(
         ...
         */
        val viewCache: SparseArray<View> = SparseArray<View>(childCount)
        var startLeftOffset = emptyLeft
        var startTopOffset = emptyTop
        if (childCount !== 0) {
            val topView: View = getChildAt(0)!!
        ...
        for (i in 0 until visibleChildCount) {
            ...
            var view: View? = viewCache[nextPosition]
            if (view == null) {
                /*
         ...
         */for (i in 0 until viewCache.size()) {
            val removingView: View = viewCache.valueAt(i)
    ...
    override fun scrollToPosition(position: Int) {
        if (position >= itemCount) {
            Timber.e(
                TAG,
                "Cannot scroll to $position, item count is $itemCount"
            )
            return
        }
    ...
    override fun scrollHorizontallyBy(
        ...
        val bottomView: View = getChildAt(mVisibleColumnCount - 1)!!

        //Optimize the case where the entire data set is too small to scroll
        val viewSpan: Int = getDecoratedRight(bottomView) - getDecoratedLeft(topView)
        ...
        delta = if (dx > 0) { // Contents are scrolling left
            ...
            if (rightBoundReached) {
                ...
                val rightOffset: Int =
                    horizontalSpace - getDecoratedRight(bottomView) + paddingRight
            ...
            //Check left bound
            if (leftBoundReached) {
                val leftOffset: Int = -getDecoratedLeft(topView) + paddingLeft
    ...
    override fun scrollVerticallyBy(
        ...
        val bottomView: View = getChildAt(childCount - 1)!!

        //Optimize the case where the entire data set is too small to scroll
        val viewSpan: Int = getDecoratedBottom(bottomView) - getDecoratedTop(topView)
        ...
        if (dy > 0) { // Contents are scrolling up
            ...
            delta = if (topBoundReached) {
                val topOffset: Int = -getDecoratedTop(topView) + paddingTop
    ...
    private fun layoutAppearingViews(
        ...
        for (extra in 1..extraCount) {
             ...
             */
            val appearing: View = recycler.getViewForPosition(extraPosition)
            addView(appearing)
    ...
    private fun layoutDisappearingView(disappearingChild: View) {
        ...
        val lp =
            disappearingChild.layoutParams as LayoutParams
    ...
    private fun layoutTempChildView(
        ...
        val layoutTop: Int = getDecoratedTop(referenceView) + rowDelta * mDecoratedChildHeight
        val layoutLeft: Int = getDecoratedLeft(referenceView) + colDelta * mDecoratedChildWidth
    ...
    private val totalRowCount: Int
        private get() {
            ...
            var maxRow: Int = itemCount / mTotalColumnCount
            //Bump the row count if it's not exactly even
